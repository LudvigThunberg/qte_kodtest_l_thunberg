...
export const SinglePost = ({ post }: ISinglePostProps) => {
  ...
  const addComment = (message: string, name: string) => {
    setComment(new Comment(message, name, post._id));
  };
  ...
  let comments = allComments.map((comment) => {
    return <SingleComment key={comment._id} comment={comment}></SingleComment>;
  });
